...
@dataclass
class Position:
    """Normalized position data for analysis."""
    ...
    gain_abs: float | None
    pe_ratio: float | None
    price_low_52w_ratio: float | None
    enrichment_missing: bool
    rating_low_confidence: bool


@dataclass
class Candidate:
    """Position candidate with reasons for action."""
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {**self.position.__dict__, "reasons": self.reasons}
...
# Position Building
# ---------------------------------------------------------------------------
def _build_position(row: dict[str, Any]) -> Position:
    """Build Position from raw row data."""
...
# ---------------------------------------------------------------------------
def _evaluate_trim_reasons(pos: Position) -> list[str]:
    ...
    if (
        pos.gain_pct is not None
        and pos.gain_pct >= THRESHOLDS["trim_gain_pct"]
    ):
        reasons.append(
            f"High weight ({w:.2%}) and strong gain ({pos.gain_pct:.2%})"
        )

    if pos.pe_ratio is not None and pos.pe_ratio >= 40:
        reasons.append(
            f"High weight ({w:.2%}) and stretched PE ({pos.pe_ratio:.2f})"
        )

    return reasons


def _evaluate_add_reasons(pos: Position) -> list[str]:
    ...
    if pos.gain_pct is not None and pos.gain_pct < 0:
        reasons.append(
            f"Good rating ({rating}), underweight ({w:.2%}), down ({pos.gain_pct:.2%})"
        )
...
def _evaluate_replace_reasons(pos: Position) -> list[str]:
    ...
    if pos.gain_pct is not None and pos.gain_pct < 0:
        reasons.append(
            f"Bad rating ({rating}) and losing ({pos.gain_pct:.2%})"
        )

    return reasons


def _evaluate_review_reasons(pos: Position) -> list[str]:
    """Evaluate reasons to review rating (bad rating but low confidence)."""
    rating = _get_rating({"rating": pos.rating})
    if rating in BAD_RATINGS and pos.rating_low_confidence:
        return [
            "Bad rating but enrichment missing (low confidence). Verify rating inputs / fundamentals."
        ]
    return []
...
def _generate_candidates(
    ...
    trim, add, replace, review = [], [], [], []

    for pos in positions:
        if reasons := _evaluate_trim_reasons(pos):
            trim.append(Candidate(pos, reasons))
        if reasons := _evaluate_add_reasons(pos):
            add.append(Candidate(pos, reasons))
        if reasons := _evaluate_replace_reasons(pos):
            replace.append(Candidate(pos, reasons))
        if reasons := _evaluate_review_reasons(pos):
            review.append(Candidate(pos, reasons))
...
def _sort_and_limit_candidates(
    ...
    trim = sorted(
        candidates["trim"],
        key=lambda c: (c.position.alloc_of_account, c.position.gain_pct or 0),
        reverse=True,
    )[:10]
    add = sorted(
        candidates["add"],
    ...
    replace = sorted(
        candidates["replace"],
        key=lambda c: c.position.alloc_of_account,
        reverse=True,
    )[:10]
    review = sorted(
        candidates["review"],
...
def _process_positions(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    top3 = [p.__dict__ for p in positions[:3]]
    top15 = [p.__dict__ for p in positions[:15]]
    ...
    return {
        ...
        "top3_positions": top3,
        "top_positions": top15,
        "candidates_trim": [c.to_dict() for c in candidates["trim"]],
        "candidates_add": [c.to_dict() for c in candidates["add"]],
        "candidates_replace": [c.to_dict() for c in candidates["replace"]],
        "candidates_review_rating": [
            c.to_dict() for c in candidates["review"]
        ],
    }
